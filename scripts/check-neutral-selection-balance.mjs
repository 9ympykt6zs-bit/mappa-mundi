...
assert.equal(first.checks.masteredDoesNotDominateWhenDueReviewExists, true);
assert.equal(first.checks.masteredRemainsEligibleWhenDue, true);
...
assert.equal(first.masteredReviewPressure.mixedDuePressure.masteredSelections, 0);
assert.equal(first.masteredReviewPressure.mixedDuePressure.nonMasteredSelections, 2000);
assert.equal(first.masteredReviewPressure.allMasteredDue.masteredSelections, 2000);
assert.deepEqual(first.masteredReviewPressure.allMasteredDue.neverSelectedItemIds, []);
