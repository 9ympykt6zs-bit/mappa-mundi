...
function renderProfile(result) {
  ...
  return `## ${result.profile.label}\n\n${result.profile.description}\n\n`
    ...
    + `- Synthetic average response time: ${summary.averageResponseTimeMs} ms (recorded evidence only; the U.S. Memory Trail planner does not rank by speed)\n`
...
const markdown = `# U.S. synthetic learner simulation report\n\n`
  ...
  + `## Scope\n\n`
  + `The eight profiles drive the production U.S. Memory Trail planner across its production-derived 50-state/50-capital curriculum. They explicitly cover a new learner, a strong/fast learner, one persistent weak item, initial struggle followed by improvement, forgetting/return, regional weakness, mixed performance, and seeded-random performance. The forgetting profile advances its injected clock by 45 days at mid-run. U.S. Memory Trail scheduling remains session-based, so elapsed time alone is not expected to make items due. A separate controlled Daily Trail completed-review probe demonstrates the current date-aware state and selection context without claiming the two planners share state.\n\n`
