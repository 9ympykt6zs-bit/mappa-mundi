...
| 6. Map Reconstruction | Weeks 9-10 | Complete (Lower 48 regions and capstone) | Ten regional reconstruction activities cover every contiguous state exactly once, with an always-available advanced Lower 48 capstone using the shared geometry and placement engine. | A child and an adult can rebuild any Lower 48 region, then attempt the full contiguous United States with zoom, pan, correction, resume, mobile interaction, and geometry-aware piece selection. |
| 7. Expedition framework | Week 11 | In progress | Build a reusable Expedition sequence model that supports introduction, atlas discovery, teaching moments, multiple mechanics, checkpoints, and a final mission. | The configuration-driven sequence/read model and reusable UI are live without an independent progress store; the Central America graduation proof passes, while narrative transition support remains. |
| 8. Across the United States Expedition | Week 12 | In progress | Create one polished Expedition crossing the country through regions, states, capitals, rivers, lakes, mountain ranges, borders, and directional reasoning. | The primary U.S. menu entry and evidence-derived nine-stop hub now compose existing modes with return routing; outside-user playtesting and final pacing/polish remain. |
