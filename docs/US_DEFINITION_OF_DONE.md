...
- [x] Journey, Daily Trail/adaptive review, U.S. Memory Trail, quizzes, Mental Map, reconstruction, and atlas/exploration function as parts of one coherent U.S. experience rather than independent progression systems.
- [x] There is a clear primary U.S. activity or orchestration layer through which the learner encounters the appropriate learning modes.
- [x] Learning evidence generated by different scored activities contributes to a shared learner model when that evidence measures the same underlying knowledge.
- [x] Moving between U.S. activities does not create contradictory, duplicated, or disconnected progress.
- [x] Activities that are intentionally non-scored, such as free atlas exploration, remain clearly connected to the overall U.S. experience without being forced into the mastery model.
- [x] No major U.S. activity requires its own incompatible or ad hoc learner-state system.

---

## 2. Learner State and Mastery

- [x] Production gameplay records the learner evidence required by the mastery system. Required evidence is not dependent on localhost-only instrumentation.
- [x] Naming, locating, and other tracked skills update the intended learner state.
- [x] Learner progress survives quitting, reloading, and returning later.
- [x] Resetting progress reliably returns the learner to the documented starting state.
- [x] Identical starting state and identical learner responses produce identical learning-state results, except where randomness is explicitly controlled by a seed or documented separately.
- [x] Repeated errors increase appropriate review pressure.
