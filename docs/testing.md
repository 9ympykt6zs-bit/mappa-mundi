...
Guided first exposure now uses the same frozen city-choice record in a target-state-only teaching mode: the capital star, its two comparison dots, and three collision-aware names appear together, while only the canonical capital is interactive. The Guided marker spec verifies teaching disclosure, inert comparison-city taps, canonical assisted evidence, and the following 150-dot unlabeled retrieval state on desktop and mobile. The overlay recalculates from true projected coordinates after map transforms; it never changes city coordinates or evidence identity.
