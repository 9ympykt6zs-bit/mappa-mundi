...
The local runtime snapshot includes the read-only U.S. continuation foundation when available: objective and family classifications, the blocking gap, and the deterministic priority reason. The primary **Learn the United States** action adds an Evidence-Driven U.S. Continuation selection trace with its chosen objective, skill/family, geographic section, considered alternatives, and destination. A targeted Guided Learning request also records its requested section, resolved section, accepted/rejected status, and fallback reason. The trace explains the recommendation without modifying canonical evidence or taking authority away from the Guided Learning scheduler.
