...
Authored state relationships remain geographic truth and continue to drive teaching and Connection content. They are separate from introduction eligibility. Hard prerequisites answer whether a feature can be taught at all: lower-48 physical targets use the first Reconstruction checkpoint, while Alaska ranges additionally require covered Alaska evidence. Geographic eligibility answers whether an unseen cohort belongs near the current Guided state frontier. Review eligibility remains based only on completed introduction, retrieval history, and due/weak evidence, so an introduced feature may be reviewed regardless of the learner's current state region. Configuration still supports explicit per-feature state prerequisites where a future technical or pedagogical dependency requires one.

## Queue and interleaving

When several physical cohorts become eligible, the queue selects the oldest ready regional stage first, then explicit Guided curriculum order, prerequisite milestone, and stable atlas-authored order. This drains an unseen eastern backlog after a learner resumes farther west instead of starving it. A requested mountain, river, or lake family uses the same geographic filter; it cannot use targeted continuation to jump to a remote unseen cohort. Selecting any unintroduced member expands the block to the remaining two- or three-target cohort. Immediate practice is treated as the in-progress continuation of that cohort and therefore runs before unrelated work. The same evidence and state produce the same queue.
