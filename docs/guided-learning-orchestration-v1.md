...
This v1 slice defines only the bounded one-retry physical-cohort remediation rule and event-based review eligibility. It does not build a general spaced-repetition system, alter ordinary Memory Trail completion rules, change state/capital Guided Learning, define route challenges, add coast instruction, replace St. Lawrence geometry, create a new gameplay engine, or replace evidence-driven top-level continuation. Introduction timing overrides and camera overrides remain available for later human playtesting rather than being guessed in advance.
