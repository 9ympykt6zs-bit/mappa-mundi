...
## Feedback label placement

Capital-location labels remain hidden until answer feedback. At that point `src/maplibre/capital-location-feedback-labels.js` measures the revealed city names in screen space and tries generic compass positions on successively wider rings around each projected marker. It places the capital first, then a selected wrong city when present, then the remaining target-state cities. Candidates stay inside the map, avoid existing labels and MapLibre controls, and strongly prefer clearance from all city markers; another city's label and leader are never allowed to cover or cross the capital star.

Nearby labels use a compact haloed card without a connector. A label moved to a wider ring or viewport-edge fallback receives a lightweight leader from the true marker coordinate to the nearest label edge. Placement recomputes through one animation frame while the map pans, zooms, or resizes. The overlay has no pointer events, does not alter the camera, and never mutates city coordinates, hit geometry, question state, evidence, or mastery. Its text is hidden from accessibility APIs because the existing feedback message already announces the result.
