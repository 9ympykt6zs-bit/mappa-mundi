...
- The supported local Playwright baseline now passes **20/20 project/test combinations** across 1440×900 desktop Chromium and the iPhone 13-sized mobile Chromium profile. Journey coverage includes start, a real session miss recorded through the same placement path, correction, activity reset, save/advance, reload/resume/continued play, and final completion persistence. A new system smoke spec covers Expedition→Atlas→Expedition routing, Daily Trail guided introduction, seeded U.S. Memory Trail weak/due adaptive review alongside new content, a submitted U.S. Connections retrieval attempt, ordinary Mental Map launch, and a playable regional Reconstruction piece bank. Acceptance flows fail on uncaught page errors and error-level console messages. This exposed and fixed an invalid MapLibre `match` expression when Atlas learning statuses were empty. The fast baseline remains **78 passed and 0 failed**. Evidence: `tests/e2e/us-journey-smoke.spec.js`, `tests/e2e/us-system-smoke.spec.js`, `src/maplibre/maplibre-activity-runner.js`, and [`docs/testing.md`](docs/testing.md).
...
- “Across the United States” is now the primary guided U.S. entry. Its configuration-driven nine-stop hub composes Atlas, the existing U.S. and capital Journeys, U.S. Memory Trail, U.S. Connections, Mental Map, regional Reconstruction, and the Lower 48 capstone without changing their mechanics or duplicating their progress. Step state is derived from Journey progress, U.S. Memory Trail progress, and canonical attempts; direct-entry evidence counts and the older direct menu paths remain. The generic framework has no storage access. Milestones 7–8 are now in progress rather than complete because narrative/pacing polish, outside-user validation, and a second-region graduation proof remain. The current fast baseline is **76 passed and 0 failed**. Evidence: `src/expedition-framework.js`, `src/across-united-states-expedition.js`, `src/expedition-ui.js`, [`docs/expeditions.md`](docs/expeditions.md), and `scripts/check-expedition-framework.mjs`.
...
- Playwright is configured for desktop Chromium and an iPhone 13-sized Chromium project. Its 20/20 supported baseline covers Journey start/input/reset/persistence/completion plus representative Expedition, Daily Trail, U.S. Memory Trail adaptive review, Atlas/navigation, Connections/Mental Map, and Reconstruction paths with runtime-error collection. Evidence: `playwright.config.js`, `tests/e2e/us-journey-smoke.spec.js`, `tests/e2e/us-system-smoke.spec.js`, and `docs/testing.md`.
...
- Expedition narrative transitions, outside-user pacing validation, and a second-region configuration/graduation proof. The framework and integrated U.S. hub now exist, but milestones 7–8 remain in progress. Evidence: [`docs/expeditions.md`](docs/expeditions.md) and `docs/us-playable-atlas-roadmap.md`.
...
Therefore the U.S. code is a reference implementation in the sense of being the deepest collection of working parts. Expedition sequencing now has a reusable configuration/read-model boundary, but the U.S. evidence adapter and the underlying mastery/mechanics remain geography-specific. A second-region Expedition graduation proof and production mastery integration are the clearest remaining work needed to claim that another country can inherit the system mainly through configuration.
