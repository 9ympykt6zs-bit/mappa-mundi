...
Architecture-graduation update on 2026-08-21:

- The bounded Central America unit (Belize, Guatemala, Honduras, El Salvador, Nicaragua, Costa Rica, and Panama) now reuses the existing North America Journey step, ordinary Memory Trail, canonical evidence repository, Progress Evidence Policy, generic canonical Progress Report core, Learning Inspector, Selection Trace, Expedition framework, and deterministic infrastructure. It adds no region-specific engine, scheduler, learner-state store, or persistence system. The production footprint is 109 region-specific lines, primarily a 99-line declarative unit configuration. The current fast baseline is **81/81** and the complete desktop/mobile Chromium baseline is **26/26**. Evidence: `src/central-america-learning-unit.js`, `scripts/check-central-america-graduation.mjs`, `tests/e2e/central-america-graduation.spec.js`, and [`docs/central-america-architecture-graduation.md`](docs/central-america-architecture-graduation.md).
...
Learning Inspector runtime surface update on 2026-08-21:

- Local development now exposes a read-only visual Learning Inspector and `window.mappaLearningInspector`. It composes the existing U.S. Memory Trail, Daily Trail, place-mastery, and canonical-evidence models without adding learner state. The panel shows all U.S. state/capital curriculum items, separate naming/locating/etc. signals, adaptive metrics and due fields, active selection reasons/traces, canonical histories and recent responses, and a bounded in-memory list of before/after canonical evidence transitions. It is not installed on non-local hosts and does not alter planner output or persisted learning state. Evidence: `src/learning-inspector-panel.js`, `src/maplibre-poc.js`, `scripts/check-learning-inspector.mjs`, and [`docs/learning-inspector.md`](docs/learning-inspector.md).
...
U.S. atlas relationship learning update on 2026-08-21:

- I2 broadens the main-menu activity to U.S. Connections and adds one fixed single-select retrieval question for each trusted atlas Census-region membership (50), international-border edge (17), and coast edge (25). The questions reuse the Mental Map engine without entering the ordinary Mental Map pool, emit semantic region/country/water response entity IDs, and persist one canonical `relationship:*` × `relationship-recall` event per submitted action. For eligible canonical-first learners, the existing optional Geographic Relationships report category appears after such evidence exists; existing learners still remain on legacy. The content reporter merges each scored question with its atlas edge and now records **50/50 assessed curated relationships** and **50/50 assessed non-capital context**, while retaining regional-balance diagnostics and all other atlas edges as separate gaps. The current fast baseline is **74 passed and 0 failed**. Evidence: `src/atlas/united-states-relationship-challenges.js`, `scripts/check-united-states-relationship-learning.mjs`, and [`docs/generated/us-content-coverage-report.md`](docs/generated/us-content-coverage-report.md).
- Physical and contextual coverage now use explicit category-appropriate normalization. Each of the 34 deliberately curated scored physical targets is counted once within its maintained feature inventory, while contextual coverage is measured against eligible states in each Census region and is 100% in all four regions. The unequal 20 mountain-range, 8 river, and 6 lake totals are documented as intentional inventories rather than equal quotas; absent physical and taxonomy categories remain visible gaps. Evidence: [`docs/generated/us-content-coverage-report.md`](docs/generated/us-content-coverage-report.md).

U.S. physical-relationship integration update on 2026-08-21:

- The next I2 slice adds 110 state relationships only for physical features already taught by the scored U.S. curriculum: 36 major-river edges, 13 Great Lakes edges, and 61 mountain-range edges. Multi-feature states never receive another true relationship as a distractor, and each question shares its canonical concept ID with the trusted atlas edge. The report now records **505 fixed assessed concepts**, reduces data-only concepts from 235 to 125, and reports no below-band state-based region/objective row under the 80%–120% diagnostic. No new geography facts, scheduler behavior, mastery thresholds, or ordinary Mental Map selection changed. Evidence: `src/atlas/united-states-relationship-challenges.js`, `scripts/check-united-states-relationship-learning.mjs`, and [`docs/generated/us-content-coverage-report.md`](docs/generated/us-content-coverage-report.md).

Adaptive verification update on 2026-08-21:

- The deterministic learner harness now has eight explicit profiles covering every named Definition-of-Done scenario: new, strong/fast, single weak item, regional weakness, initial struggle followed by improvement, return/forgetting, mixed, and seeded-random. Synthetic response time is recorded as evidence but does not affect the U.S. Memory Trail planner. A separate neutral-state verifier holds all 100 items at identical mastery/scheduling state and runs 1,000 seeded cumulative-review plans for exactly 10,000 selections. Every item was selected; regional shares deviated from eligible shares by only −2.00% to +0.88%; state/capital shares differed by less than 1%; and identical inputs replay exactly. The same report now drives the extracted production prompt selector for 10,000 ordinary-review prompts and 10,000 early-chunk prompts: locating/identifying shares exactly matched their intentional 50/50 and 70/30 targets, with neither objective starved. Controlled planner pressure also selected 0 future-due mastered items while 30 due review items were available, then selected all 100 mastered items across the probe when they were due. Matched-seed and long-horizon reports continue to demonstrate increased weak-item/weak-region pressure and lapsed-item recovery without changing planner thresholds or behavior. The current fast baseline is **77 passed and 0 failed**. Evidence: [`reports/us-learning-simulations.md`](reports/us-learning-simulations.md), [`reports/us-neutral-selection-balance.md`](reports/us-neutral-selection-balance.md), [`reports/us-matched-seed-simulations.md`](reports/us-matched-seed-simulations.md), and [`reports/us-long-horizon-mastery.md`](reports/us-long-horizon-mastery.md).
- Cumulative U.S. Memory Trail review now reserves at most one of ten slots for the longest-waiting eligible due item outside the ordinary adaptive plan. The other nine slots keep the existing priority; the lane does not alter eligibility, prerequisites, cooldowns, or new-item progression, and remains empty when no due item needs it. In a deterministic ten-weak-item probe, nine weak items retained review pressure each session while all twenty waiting due items received the fairness lane. Neutral regional/type balance and future-mastered recession remain green. Matched-seed and 200-session outputs were regenerated and now expose `fairness-review` counts and changed trajectories explicitly. The current fast baseline is **78 passed and 0 failed**. Evidence: [`docs/us-memory-trail-fairness.md`](docs/us-memory-trail-fairness.md), `scripts/check-united-states-memory-trail-fairness.mjs`, [`reports/us-neutral-selection-balance.md`](reports/us-neutral-selection-balance.md), [`reports/us-matched-seed-simulations.md`](reports/us-matched-seed-simulations.md), and [`reports/us-long-horizon-mastery.md`](reports/us-long-horizon-mastery.md).

- The production Memory Trail introduction boundary now has a pure, directly tested guard: every unexposed current-window target receives guided exposure and a completed guided tap before retrieval branches can select it. Guided interactions return before retrieval counters update and canonical storage labels them `assisted`, so exposure cannot masquerade as successful assessment. The current fast baseline is **77 passed and 0 failed**. Evidence: `src/memory-trail-introduction-guard.js` and `scripts/check-unseen-introduction-guard.mjs`.

Supported browser/E2E baseline update on 2026-08-21:

- The supported local Playwright baseline now passes **26/26 project/test combinations** across 1440×900 desktop Chromium and the iPhone 13-sized mobile Chromium profile. In addition to the U.S. Journey and system flows, it covers the Central America graduation path, live Journey evidence, shared Inspector and Progress Report, and ordinary Memory Trail Selection Trace. Acceptance flows fail on uncaught page errors and error-level console messages. The current fast baseline is **81 passed and 0 failed**. Evidence: `tests/e2e/us-journey-smoke.spec.js`, `tests/e2e/us-system-smoke.spec.js`, `tests/e2e/central-america-graduation.spec.js`, and [`docs/testing.md`](docs/testing.md).

Learner-state/reset authority update on 2026-08-21:

- Canonical evidence is now explicitly the durable shared history for reporting, orchestration, and equivalent cross-mode evidence, while each mode may retain distinct scheduler/resume state. Activity, Daily Trail, U.S. Memory Trail, Journey, and Reconstruction resets remain narrowly scoped and do not delete canonical events. A separately confirmed **Reset All Learning Progress** action clears a fixed nine-store allowlist covering canonical evidence, its Progress Report cohort metadata, Journey/activity completion, both adaptive trails, place mastery, and Lower 48 reconstruction resume state, then reloads to remove stale in-memory state. Difficulty, map/study/audio settings, onboarding, mute state, developer presentation configuration, and unknown preference keys survive. Focused checks pin the manifest, deterministic/idempotent behavior, failure continuation, scoped retention, and preference preservation; Playwright verifies the UX and reload behavior on both configured viewports. The current fast baseline is **79 passed and 0 failed** and the browser baseline is **24/24**. Evidence: `src/learning-progress-reset.js`, `scripts/check-learning-progress-reset.mjs`, `tests/e2e/us-system-smoke.spec.js`, and [`docs/learning-progress-reset.md`](docs/learning-progress-reset.md).

Adaptive starvation acceptance update on 2026-08-21:

- The U.S. Memory Trail's reserved cumulative-review fairness slot and Daily Trail's existing old-section review lane now have complementary deterministic multi-session proofs. A persistently failed item retains remediation pressure while every other eligible waiting due item is eventually selected; future-due mastered items continue to recede and new-item introduction is unchanged. This closes the adaptive-system starvation criterion without changing Daily Trail selection behavior. The fast baseline now contains **80 checks**. Evidence: `scripts/check-united-states-memory-trail-fairness.mjs`, `scripts/check-daily-trail-starvation-protection.mjs`, and [`docs/testing.md`](docs/testing.md).
...
The strongest current evidence remains at the planner/data/engine level: the U.S. curriculum, atlas, Mental Map, reconstruction, Daily Trail selection, persistence helpers, and mastery-store helpers have passing Node assertion scripts. Browser coverage now verifies representative entry and progression paths across Journey, Daily Trail, U.S. Memory Trail, Expedition/Atlas, U.S. Connections/Mental Map, and Reconstruction on desktop and mobile-sized Chromium. Deterministic hooks are still used where precise state setup or completion is required; the suite does not establish real-device map feel, drag behavior, audio quality, camera framing, accessibility, or visual polish.
...
- “Across the United States” remains the primary guided U.S. entry. Its configuration-driven nine-stop hub composes Atlas, the existing U.S. and capital Journeys, U.S. Memory Trail, U.S. Connections, Mental Map, regional Reconstruction, and the Lower 48 capstone without changing their mechanics or duplicating their progress. The generic framework has no storage access, and the Central America graduation now proves its learning-unit, evidence, report, Inspector, trace, and test boundaries in a second region. Milestones 7–8 remain in progress only for narrative/pacing polish and outside-user validation. Evidence: `src/expedition-framework.js`, `src/across-united-states-expedition.js`, [`docs/expeditions.md`](docs/expeditions.md), and [`docs/central-america-architecture-graduation.md`](docs/central-america-architecture-graduation.md).
...
- Playwright is configured for desktop Chromium and an iPhone 13-sized Chromium project. Its 26/26 supported baseline covers Journey start/input/reset/persistence/completion; representative U.S. Expedition, Daily Trail, U.S. Memory Trail adaptive review, Atlas/navigation, Connections/Mental Map, and Reconstruction paths; and the Central America architecture-graduation path with runtime-error collection. Evidence: `playwright.config.js`, `tests/e2e/us-journey-smoke.spec.js`, `tests/e2e/us-system-smoke.spec.js`, `tests/e2e/central-america-graduation.spec.js`, and `docs/testing.md`.
- The deterministic `completeCurrentActivity()` hook directly marks every current target complete and invokes completion handling. Separate test-only helpers now drive one incorrect and one correct placement through the normal placement path and inspect/reset attempt state; adaptive-review coverage inspects the actual active U.S. Memory Trail plan after seeded persisted evidence. Map hit testing and long browser-played adaptive sessions remain outside this layer. Evidence: `installMappaTestApi()` in `src/maplibre-poc.js`.
- The 78 standalone checks provide broad low-level coverage through the consolidated `npm test` command. They remain executable assertion scripts rather than a uniform test framework, and the repository still has no executed-JavaScript coverage report or `.github` CI workflow. Evidence: `package.json`, `scripts/run-fast-checks.mjs`, `scripts/check-*.mjs`, and the absence of `.github` files.
...
- **Adaptive behavior end to end:** Playwright now proves that a persisted weak/due U.S. Memory Trail item enters the active adaptive review plan alongside new content, but it does not play long Daily Trail/U.S. Memory Trail sessions, reload mid-session, or validate slow-correct adaptation through map input.
- **Atlas integration:** Playwright proves Expedition launch/return and visible Atlas overview rendering, but not state-profile accuracy after real learning sessions.
- **Mental Map and Map Reconstruction UX:** Playwright submits one Connections attempt, opens ordinary Mental Map, and loads a regional Reconstruction piece bank on both viewports. Geometry feel, full reconstruction completion/persistence, and real touch dragging remain unverified.
...
- Expedition narrative transitions and outside-user pacing validation. The framework, integrated U.S. hub, and bounded second-region graduation proof now exist, but milestones 7–8 remain in progress for experience polish. Evidence: [`docs/expeditions.md`](docs/expeditions.md), [`docs/central-america-architecture-graduation.md`](docs/central-america-architecture-graduation.md), and `docs/us-playable-atlas-roadmap.md`.
...
- Deep E2E coverage for complete Daily Trail/U.S. Memory Trail sessions, Atlas state updates, full Mental Map/Reconstruction progression, audio, camera behavior, accessibility, real devices, and non-U.S. journeys.
...
The bounded Central America graduation now demonstrates that conventional location and identification learning can inherit the U.S. reference system mainly through configuration while reusing canonical evidence, Progress Report, Inspector, Selection Trace, Expedition, Journey, Memory Trail, and deterministic tests. U.S. Memory Trail, Atlas relationship queries, Mental Map, and Reconstruction remain intentionally U.S.-specific mechanics rather than a claim that every advanced mode is already geography-generic. Production multidimensional mastery integration remains the clearest architectural gap.
